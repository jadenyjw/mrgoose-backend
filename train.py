...
conv_1 = Convolution2D(conv_depth_1, (kernel_size, kernel_size), padding='same', activation='relu', 
kernel_regularizer=regularizers.l2(0.01))(inp)
#conv_2 = Convolution2D(conv_depth_1, (kernel_size, kernel_size), padding='same', activation='relu', 
#kernel_regularizer=regularizers.l2(0.01))(conv_1)
pool_1 = MaxPooling2D(pool_size=(pool_size, pool_size))(conv_1)
drop_1 = Dropout(drop_prob_1)(pool_1)
# Conv [64] -> Conv [64] -> Pool (with dropout on the pooling layer)
conv_3 = Convolution2D(conv_depth_2, (kernel_size, kernel_size), padding='same', activation='relu', 
kernel_regularizer=regularizers.l2(0.01))(drop_1)
conv_4 = Convolution2D(conv_depth_2, (kernel_size, kernel_size), padding='same', activation='relu', 
kernel_regularizer=regularizers.l2(0.01))(conv_3)
...
model.fit_generator(datagen.flow(train_X, train_Y, batch_size=10),
                     epochs=num_epochs, steps_per_epoch=len(train_X) / 10, validation_data=valgen.flow(val_X, 
val_Y), validation_steps=len(val_X) / 10)
